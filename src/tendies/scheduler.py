...
from .models import Company, MarketClose, ServerState
...
class TickScheduler:
    ...
    async def synchronize_guild(
        ...
        async with self.bot.db.session() as session:
            ...
            if state is None:
                return []
            if not self.accelerated and state.game_day > target:
                # A playtest in accelerated mode (or a $setday) can leave the
                # cursor in the future. In calendar mode that cursor would never
                # reach "today" again, so no date would ever close — silently.
                # Snap back to today WITHOUT settling anything: no payouts, no
                # invented attendance, just the cursor.
                #
                # Unless a *settled* date is already later than local today: a
                # westward GAME_TIMEZONE change can move "today" behind dates
                # this guild has already closed. Snapping then would rewind onto
                # a settled date and re-close it — double-advancing vesting and
                # overwriting its immutable market closes. Leave the cursor be;
                # local today will catch up on its own.
                #
                # market_closes is the only per-guild record written *only* by a
                # close, so it is the trustworthy "this date is settled" marker.
                # (It covers private companies only; a guild that has never had
                # one has no quotes or vesting to damage anyway.)
                newest_close = await session.scalar(
                    select(func.max(MarketClose.game_day))
                    .select_from(MarketClose)
                    .join(Company, Company.id == MarketClose.company_id)
                    .where(Company.guild_id == guild_id)
                )
                if newest_close is not None and newest_close >= target:
                    log.warning(
                        "Guild %s calendar cursor is ahead of local today "
                        "(game_day=%s, today=%s) but %s is already settled; "
                        "NOT snapping back — rewinding would re-close a settled "
                        "date. The cursor stays put until today catches up.",
                        guild_id,
                        state.game_day,
                        target,
                        newest_close,
                    ...
                    return []
                log.warning(
                    "Guild %s calendar cursor is in the future (game_day=%s, "
                    "today=%s); snapping back to today without settling. "
                    "No wages, production, or attendance are generated by this.",
                    guild_id,
                    state.game_day,
                    target,
                )
                state.game_day = target
                state.weekday = gameday.weekday_name(target)
                return []
    ...
    async def _ping_forgetful(self, guild_id: int, user_ids: list[int]) -> None:
        ...
        try:
            await channel.send(
                ...
                # The bot suppresses mentions globally (bot.py), so this send —
                # the whole point of which is to ping — opts back in, scoped to
                # exactly the users who asked for the reminder.
